...
try:
	...
	###########################################################################################
		...
		if y[9] == 'Redhat':
			w = commands.getstatusoutput("sudo ln /var/lib/libvirt/images/rhel7.1.qcow2 /var/lib/libvirt/images/redhat-"+ str(receivedId)+"-"+str(y[16])+".qcow2 ")
			print w
...
#			print "<br /><br />"
			...
			print "prakshi"
			print f1
			zuts = commands.getstatusoutput("sudo replace \"listen='192.168.42.151'\" \"listen='0.0.0.0'\" -- /etc/libvirt/qemu/"+name+".xml")
			zuts = commands.getstatusoutput("sudo replace \"address='192.168.42.151'\" \"address='0.0.0.0'\" -- /etc/libvirt/qemu/"+name+".xml")
			print zuts
...
#	x = commands.getstatusoutput("virsh start trail")
			...
			x = commands.getstatusoutput("sudo /root/Downloads/websockify-master/websockify.py -D "+f5+" 192.168.42.151:"+f1)
			print f1
	...
	#		print x
			...
			w = commands.getstatusoutput("sudo ln /var/lib/libvirt/images/debian.qcow2 /var/lib/libvirt/images/debian-"+ str(receivedId)+"-"+str(y[21])+".qcow2 ")
			print w
			cursor.execute(insertion)
			print name
			x = commands.getstatusoutput("sudo virsh destroy "+name)
...
#	x = commands.getstatusoutput("virsh start trail")
	...
	#		print x
			...
			w = commands.getstatusoutput("sudo ln /var/lib/libvirt/images/windows.qcow2 /var/lib/libvirt/images/windows-"+ str(receivedId)+"-"+str(y[20])+".qcow2 ")
			print w

			print "hello"
			name = "Windows-" + y[1]+'-' +str(y[20])
			insertion = 'update  USER set Win_COUNTER = Win_COUNTER +1 where ID =%d;' % (receivedId)
	
			cursor.execute(insertion)
			print name
			x = commands.getstatusoutput("sudo virsh destroy "+name)
			x = commands.getstatusoutput("sudo chmod 777 /etc/libvirt/qemu/ -R ;sudo touch /etc/libvirt/qemu/"+name+".xml;sudo cp /etc/libvirt/qemu/windows.xml	/etc/libvirt/qemu/"+name+".xml")
#			print x
			#lets replace the name first
			x = commands.getstatusoutput("sudo cat /etc/libvirt/qemu/{0}.xml  | sudo grep name | sudo sed -i 's/<name>windows</<name>{0}</' /etc/libvirt/qemu/{0}.xml".format(name))
#			print x
			x = random.sample('1234567890abcdef1234567890abcdef1234567890acbdef',6)
#			print x
			mac ="30:f9:ed:"	
...
#			print x
			print y[21]
			var = "windows-"+receivedId+"-"+str(y[20])+".qcow2"
			print var
			x = commands.getstatusoutput("sudo replace \"windows.qcow2\" \""+var+"\" -- /etc/libvirt/qemu/"+name+".xml")
			print x
#			print "hii"
#			print "<br /><br />"
			e =random.randint(5900,65535)
