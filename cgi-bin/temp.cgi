...
#		print a
		x = commands.getstatusoutput("sudo docker images | awk '{print $1 ;}' | grep -i projectf_" + y[17])
#		print x[1]
		l = []
		k = int(y[18])
#		print k
...
#			print r
			r = commands.getstatusoutput('sudo echo "echo -e \''+y[3]+'\n'+y[3]+'\' | passwd  '+y[1]+' \n" >> /dockerFile/user.sh')
#			print r
#			print "hello"
			r = commands.getstatusoutput("sudo docker run -dti --privileged -v /dockerFile/user.sh:/dockerFile/user.sh -p "+str(receivedId)+"69"+str(j)+":4200 " + x[1] + " bash")
#			print r[1]
#			print x[1]
#			print r[1]
			if instanceName == "kali":

				i = commands.getstatusoutput("sudo docker exec -t "+r[1]+" service ssh restart")
#				print i
				i = commands.getstatusoutput("sudo docker exec -t "+r[1]+" service shellinabox restart")

#				print i
				i = commands.getstatusoutput('sudo docker exec -t '+r[1]+'    /dockerFile/user.sh')
#				print i
			else:
				ip = commands.getstatusoutput("sudo docker exec -t "+r[1]+" hostname -i")
#				print ip
				myip = ip[1].strip("\r")
				i = commands.getstatusoutput("sudo docker exec -t "+r[1]+" sed -i 's/172.17.0.2/"+myip+"/'  /etc/sysconfig/shellinaboxd")
#				print i

				i = commands.getstatusoutput("sudo docker exec -t "+r[1]+" /etc/init.d/shellinaboxd restart")
				i = commands.getstatusoutput("sudo docker exec -t "+r[1]+" /etc/init.d/sshd restart")
...
#		print y[18]
		k = int(y[18])
		for p in range(k):
			print'''			<form target="_blank" style="display:inline;" method="post"  action="https://192.168.42.151:'''+str(receivedId)+'''69'''+str(p)+'''">
			...
			<input type="hidden" name="counter" id="{0}" value={0} />

			<input  style="margin-top:5px;" type="submit"  value = "Launch-{0}" ></input>
